...
\textbf{Support for the HL2 I/O board} is as follows: if the presence of an HL2 I/O board has been detected,
...
\begin{align}
C = (int) \left\{ 0.5 + 15.47 * ln(f / 18748.1)\right\}
\end{align}
...
(14/15 for first-generation RadioBerries, and 17/18 for second-generation ones)
are actually available, so if \pH is compiled with GPIO support and detects a RadioBerry ``hat''
...
\textbf{A note on the RadioBerry.} If \pH detects that it runs a RadioBerry device that is GPIO-connected,
it will only use GPIO lines 14 (for \bltt{CWL}) and 15 (for \bltt{CWR}) if \texttt{No Controller} is
selected. Any other controller choice will not work in this case anyway. This allows RadioBerry users
to compile \pH with GPIO enabled and use these two lines to attach a paddle and  work with the iambic
keyer built into \pH.

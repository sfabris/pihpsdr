...
TS-2000. See the last section of this Appendix for some special CW features.
...
\section{Special characters in CAT CW}

CW can be sent via the Kenwood CAT command \texttt{KY} as usual. The opening and closing brackets, \texttt{[} and
\texttt{]}, which are not normally used in CW, have been given a special meaning. This is an extension
of the Kenwood protocol. Currently implemented is
a possibility to temporarily change the CW speed for certain characters, and to send characters without
inter-character separation. An opening bracket starts the ,,special mode'' and a closing bracket ends it.
Any special mode automatically ends if the radio goes RX after the message has been sent, or if a
Morse key has been hit and CAT CW is terminated.

\textbf{Speed change.} The two-character sequence \texttt{[+} temporarily increases the speed by 25\%,
 while the sequence 
\texttt{[-} temporarily reduces the speed by 25\%. For example, if the CW speed is 24 wpm and the text


\texttt{DL0AAA [+ENN] [-123]}

is sent via CAT CW, then the call sign will be sent at nominal (24 wpm) speed, while the report (enn) will be sent
at 30 wpm and the serial number (123) will be sent at 18 wpm. The inter-word separations are those of
the nominal speed (24 wpm) since they are outside of the brackets.
Such speed changes are often used in contests,
where the report is largely redundant and one wants to be nice to stations which have difficulties
with the normal contest speed. From this example, it is clear that
such in-line speed changes are primarily used if piHPSDR is keyed by a contest logger through CAT CW
messages. The nominal speed can be changed either via the CAT command \texttt{KS} or via
the \bltt{CW} menu. The nominal speed change becomes effective immedeately, which can be in the middle
of an on-going CW transmission. This will not change the speed if it is made temporarily faster or slower,
since once one goes ,,slow'' or ,,fast'' this speed is fixed until a closing bracket or the end of the
transmission is seen. 

\textbf{Character joining.} The two-character sequence \texttt{[.}  leads to a suppression of inter-character spacing until
a closing bracket is met or the radio goes RX at the end of the transmission.
For example, the string \texttt{[.ar]} is transmitted exactly as if it were a plus sign. Another example
for a joined character is \texttt{[.sk]} which is sometimes used to mark the end of a transmission.    
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\section{GPIO lines used by piHPSDR}
...
\section{A note on the ,,BookWorm'' OS and the Pi5 GPIO}

Both, BookWorm and the Pi5, are a little different from BullsEye/Pi4 if it comes to the GPIO.
 It is planned that sooner or later all what follows here
will be done automatically, so take this note as useful hint for the time being.

\begin{itemize}
\item{
The install procedure puts two additional lines at the end of the file \texttt{/boot/config.txt},
namely
...
These lines take care that
all the GPIO lines used by piHPSDR are switched to input with a pull-up upon booting the computer.
This is only necessary if the hardware attached to the GPIO does not feature external pull-up
resistors. Note that the Controller2 also uses GPIO line 15, but this one has external pull-up
resistors and lines 14/15 are used by serial interface if it is enabled.

This configuration file has been moved in BookWorm to another location,
 \texttt{/boot/firmware/config.txt}. If your hardware lacks pull-up resistors,
  you have to modify the file 
 and add the two lines manually. Although I have not tried it, I guess this
is also necessary if you run BookWorm on a Pi4.}

\item{The Pi5 has a GPIO layout different from the Pi4. Fortunately, the external library (libgpiod) 
handles most of this for piHPSDR. While the mapping of GPIO  line numbers to GPIO header pins
has not changed, the GPIO lines on the Pi5 are now associated
with GPIO chip \#4, while the were associated with GPIO chip \#0 on the Pi4.

 For this reason, on a Pi5 one has to change one line in the
source code file \texttt{src/gpio.c}. Somewhere around line 230, there is a string \texttt{"/dev/gpiochip0")}
which must be replaced by \texttt{"/dev/gpiochip4"} on a Pi5.}

\item{It seems that GPIO line 20 is not available to piHPSDR on a Pi5, 
since it is used there for a special purpose (power switch).
Unfortunately, this line is used by all controllers so the connection to GPIO20
has to be re-wired to GPIO14 (the only line not used by any of the controllers).
Then, the tables in gpio.c have to be modified accordingly. To this end, locate the number "20" in
\texttt{src/gpio.c} 
in the line range 240--300, and replace it by "14". Locate further the three places where the value 14 is
assigned to \texttt{PTTIN\_LINE}, and replace this by the number $-1$, simply because GPIO14 is no longer
...
(marked by the red arrows). Take care that the SerialPort is also disabled (as shown here),
since the Controller2 uses GPIO lines normally used by the serial interface. Then, restart the computer.
...
If you want to use the GPIO with BookWorm and/or Pi5, see the end of Appendix \ref{sec:gpio}.}
...
\section{Install piHPSDR from the internet}
...
release version is safer since this is more stable, and will only be updated for simple bug fixes.
New features will be included and accumulated in the development branch until eventually a new
release (with version 2.3) is made in the future. This manual corresponds, however, to the
,,current snapshot'', that is, the development version.
}
}
\end{center}
...
\section{Checking the SoapySDR installation}
...
Starting with BookWorm, all piHPSDR testing is done with the 64-bit OS version so it is highly recommended
to switch to 64 if you use BookWorm. This especially holds if you run piHPSDR on the new Raspberry Pi5 
which requires BookWorm.

Both, BookWorm and the Pi5, are a little different from BullsEye/Pi4 if it comes to the GPIO.
You can skip the rest of
this section if you are not using the GPIO. It is planned that sooner or later all what follows here
will be done automatically, so take this note as useful hint for the time being.
...
The file has been moved in BookWorm to
 \texttt{/boot/firmware/config.txt}. If your hardware lacks pull-up resistors,
  you have to modify the file 
 and add the two lines manually. Although I have not tried it, I guess this
is also necessary if you run BookWorm on a Pi4.}
...
handles most of this for piHPSDR. While the mapping of GPIO  line numbers to GPIO header pins
has not changed,
the  remaining difference is  that the GPIO lines on the Pi5 are associated
with ,,gpiochip4'', while the were associated with ,,gpiochip1'' on the Pi4.

 For this reason, on a Pi5 one has to change one line in the
source code file \texttt{src/gpio.c}. Somewhere around line 230, there is a string \texttt{"/dev/gpiochip1")}
which must be replaced by \texttt{"/dev/gpiochip4"} on a Pi5.}

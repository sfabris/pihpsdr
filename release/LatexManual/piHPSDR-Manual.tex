...
should stay at +0 dB. For most setups, this also applies if one uses the TX compressor since this will
automatically activate an auto-leveler. However, if no compression etc. is used in the TX chain, then
it is usually required to advance the \rett{Mic} slider to about +10 dB. The reason is simply, that
for most audio gear one takes care that one stays below -10 dBFS (FS stands for full scale). The best
way to level the microphone in SSB modes, is to display the TX peak ALC value in the meter. Then one
decreases the \rett{Mic} gain if the value if close to zero, and one increases the \rett{Mic} gain if
the peak ALC value is substantially below zero during normal speech. Not only your transmission will
sount ``thin'' if the \rett{Mic} gain is too low, adaptive predistortion (PURESIGNAL) also requires
that TX ALC values peak close to zero.
...
be further amplification in the TX  chain through the \bltt{Mic} slider.
...
\textbf{Support for the HL2 I/O board}
...
\textbf{``TX latency'' and ``PTT hang time''}

There has been much discussion about which are the optimal values for these parameters. \pH has not
user interface for there but sets the TX-latency to 40 msec and the PTT hang time to 20 msec.
...
seems to be that a value of 20 msec is somewhat small, so \pH sets the TX latency to 40 msec. My measurements
indicate that the buffer can hold about 75msec of TX data so a value of 40 msec leaves enough headroom.
...
\textbf{HL2 parameters/warnings displayed in the panadapter}
...
the usual warning, also an underrun or an overflow of the FPGA TX sample buffer is reported.

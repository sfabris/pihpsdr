...
The Pluto is
operated via the SoapySDR interface. It can transmit at 2.3 GHz so for transmitting, so one can
simply choose the "2300" (13 cm) band. For receiving in the 10 GHz band, one uses a so-called
...
Duplex mode implies that the receiver continues to work while transmitting so you can watch your own signal.
The TX spectrum scope then appears during transmitting in a small separate window. SAT mode
finally takes care that changes of the RX (VFO-A) frequency are applied with the same sign and
magnitude to VFO-B, so your transmitting frequency follows your RX frequency. The RIT value
is used to correct for small local oscillator variations in the QO-100. One could have used
the \rett{LO Err} field in the \bltt{XVTR} menu for this purpose, but I was told that
this frequency is varying on a daily basis so using RIT is easier than setting the
LO correction in the \bltt{XVTR} menu.
...
compilation of piHPSDR. By default (and if piHPSDR has been put onto you computer by binary
installation), ALSA is active.
...
only consuming a lot of CPU power (on both sides), but also puts a high load on the network
connection. While using VNC is perfectly OK for compiling and installing software, it is
{\color{red} highly recommended} to shut down RealVNC on the RaspPi when connecting to a radio.
Many users have reported audio cracks and other signs of failures in the data stream between the
radio and the RapsPi when using RealVNC. An indication of network or CPU congestion is, if
the audio problems get worse when the sample rate changes, say, from 48k to 96k to 192k.
It has also been reported that when using VNC, the situation gets worse then the piHPSDR window
is made larger, which is also an indication that problems are rooted in the resources
consumed by the VNC connection.
}
}
\end{center}
...
such that you can execute ,,sudo'' commands in the terminal window. 
Some Linux distros maintain a ,,sudoers'' file where users that are allowed to execute
...
will have administrator privileges by default, and no password is asked when
executing \grtt{sudo} commands.
}
}
\end{center}

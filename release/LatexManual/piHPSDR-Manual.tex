...
For each band, the band stack is collection of operating frequencies. The idea is that
you can have preferred or recently visited frequencies to which you can easily come back.
What is actually stored is the frequency and the band, the mode (e.g. USB or FMN),
the filter, whether CTUN is enabled or disabled, and the CTCSS setting (the latter
only becomes effective for the FMN mode).
...
data from any slot. Besides the frequency and the mode, the filter used and the CTCSS setting
is part of the memory slot.
...
The default functions associated with the encoders are printed
on the enclosure for the G2 frontpanel and the Controller2. So you might not wish
to re-assign different functions, but at least you can.
...
The default functions associated with the push buttons are printed
on the enclosure for the G2 frontpanel and the Controller2. So you might not wish
to re-assign different functions, but at least you can.
...
{\color{red} \textbf{For Non-RaspPi Linux systems:} The whole installation procedure depends on that your user account is an administrator account,
...
(you own the computer) you should have administrator privileges.}
...
{\color{red} The whole installation procedure depends on that your user account is an administrator account,
such that you can execute ,,sudo'' commands in the terminal window. You may be asked the administrator
password. If your user account does not qualify for administrator work, then you are simply not allowed
to install the ,,HomeBrew'' universe which is the prerequisite for compiling piHPSDR. In normal cases

...
settings, equaliser settings, and TX compressor settings
 ...
 To this end, the settings given below are combined in a ,,RX/TX profile'' that
 is associated with a certain mode or a certain group of modes. Modes grouped
 together are USB/LSB/DSB, CWU/CWL, and DIGU/DIGL. This has been done because
 if one has figured out optimal settings for e.q. the TX and RX equalisers for LSB,
 one most likely wants to have the same settings automatically for USB as well.
 When the mode of the VFO controlling RX1, RX2, or the transmitter is changed,
 the settings for that VFO/receiver/transmitter will be loaded from the profile.
...
A profile is not only loaded if a mode is explicitly changed, but also when the mode changes
because one has copied/swapped VFOs, changed the band or the band stack, or recalled a memory slot.

A receiver- or VFO-speficic setting will be updated in the profile if it is changed and if this
change applies to the active receiver. It is then stored in the profile associated with the mode
(group)
of the active receiver. Changing a transmitter-specific setting (TX equaliser, CFC, or DEXP) always
leads to profile update of the profile associated with the mode (group) of the VFO that controls the
transmitter frequency (that is, VFO-A if not working  split, and VFO-B if working split). The updated
profile will only become effective upon the next mode change to that mode.

Note that the concepts of mode groups implies that the profile associated with, say, LSB is
automatically updated if the USB profile is updated, such that the profiles of the mode  within
a mode group (although stored separately for historical reasons) will be identical.
...
=======
\rett{Bypass ADC1 RX filters}. This box is only shown if the RF front-end for ADC1
features a filter board (Anan-7000/8000 and G2 radios),
and if the ALEX filter board
is selected (see the \bltt{Radio} menu). If checked, the filters in the RF front-end for ADC1
>>>>>>> 328c9d5b18539b1a263ded844858e53b801486ba
...
changing the split status or the actice receiver.
...
\action{Mute}{MUTE}{B}{Toggles the ,,mute'' state of the active receiver. If a receiver is muted,
<<<<<<< HEAD
zero-amplitude audio output is sent to the radio via the HPSDR protocol.
This has only an effect for Protocol-1 and Protocol-2 (not for SoapySDR), and only affects headphones/speakers
connected to the radio. Local audio (to a sound card or a virtual audio cable)
is not affected. The main use of this action is
to mute a radio-connected headphone while doing digi mode via local RX output.}

\action{Mute RX1}{MUTE1}{B}{Toggles the ,,mute'' state of  RX1. If a receiver is muted,
zero-amplitude audio output is sent to the radio via the HPSDR protocol.
This has only an effect for Protocol-1 and Protocol-2 (not for SoapySDR), and only affects headphones/speakers
connected to the radio. Local audio (to a sound card or a virtual audio cable)
is not affected. The main use of this action is
...
to mute a radio-connected headphone while doing digi mode via local RX output.}
=======
zero-amplitude audio output is sent to the radio via the HPSDR protocol. This option has no
effect on local audio (e.g. sent to a sound card).}
...
$\phantom{\Big|}${\large Notes} & \multicolumn{1}{|p{11cm}|}{p$_1$=0: auto-reporting disabled, p$_1$=1: enabled} \\
 ...
 & \multicolumn{1}{|p{11cm}|}{For p$_1$=1, only frequency changes are sent via FA/FB commands.} \\
 & \multicolumn{1}{|p{11cm}|}{For p$_1$>1, mode changes are also sent via MD commands.} \\
>>>>>>> 328c9d5b18539b1a263ded844858e53b801486ba
\bottomrule
\end{tabular}
\end{center}
...
$\phantom{\Big|}${\large Notes} & \multicolumn{1}{|p{11cm}|}{p$_1$=0: RX1 not muted, p$_1$=1: muted.} \\
 & \multicolumn{1}{|p{11cm}|}{This only affects the audio sent to the radio via the HPSDR protocol.} \\
>>>>>>> 328c9d5b18539b1a263ded844858e53b801486ba
\bottomrule
\end{tabular}
\end{center}
...
If you have a RedPitaya and no autostart of the SDR application there, and not compiled \pH
...
=======
The libinstall command installs the
 \texttt{HomeBrew} universe, and the installation starts with asking you for the administrator
>>>>>>> 328c9d5b18539b1a263ded844858e53b801486ba

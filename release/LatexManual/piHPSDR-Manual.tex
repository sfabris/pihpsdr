...
radio on the world
provided it is on the internet. I have been told that the original HPSDR standard states that a broadcast packet
\textit{must} be used, so radios strictly adhering to that standard
won't reply to ``routed'' discovery packets. The radios I have tested (ANAN-7000, RedPitya StemLab, HermesLite-II) all
allow being discovered by such non-broadcast packets, so using the IP addr might become a
preferred option in your shack if the radio is configured to have a fixed IP address, since this
speeds up the start of the radio considerably.

In rare cases (the RedPitaya StemLab case is the only one I know of) it is  possible to use the TCP protocol
rather than the standard UDP one for communicating with the radio. A ``TCP discovery'' will wait for three
seconds if there is no response (this is the case for most radios). Therefore, trying to
connect via TCP is only done if the \rett{Enable TCP} box is checked.
...
the small \rett{MicLvl} bar. The audio level is in dB from $-40\ldots0$ dB. Note that
...
is using a WLAN. I mention this because my attempts using WLAN for connection to the \textit{radio}
(that is, for the baseband IQ data) were not entirely satisfactory.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\item{The equalizer (TXEQ)}
\item{The auto-leveller (AUTOLVL)}
\item{The continous frequency compressor (CFC)}
...
because of the FM preemphasis, this is explained in chapter \ref{sec:txmenu}, set the
explanation of the \rett{FM PreEmp/ALC} button.

\textbf{Start of the TX audio chain}
...
gear is leveled such that you stay significantly below FS during normal speech. Note that once you
...
It is clearly of interest how the peak input levels are, and this is displayed in the \texttt{MicLvl}
...
the microphone stand) could also be useful.

\textbf{The downward expander (DEXP), a flexible noise gate}
...
\textbf{MicGain and peak TX ALC value}
...
processor (CMPR) or the adaptive predistortion (PURESIGNAL) won't work.
...
At the very end of the TX chain, the baseband signal is converted again from a floating point
to an integer representation (in the range $\pm 2^{23}$) so it is critical to ensure that the baseband
signal (in floating point representation) does not exceed $\pm 1.0$. This is where the ALC (automatic
level control) at the end of the TX chain sets in: it attenuates the baseband signal such that it
...
\textbf{The TX equaliser}

Then comes the TX equaliser which is less important (in my view, I personally do not
use it). It is described in chapter \ref{sec:eqmenu} and I guess any reader knows what
an equaliser is. What is relevant here is, that the filtering
involved in the equaliser changes the peak signal amplitude. While this could be compensated
...
\textbf{The continuous frequency compressor (CFC)}
...
\textbf{The speech processor (CMPR)}
...
The additional filtering that has to be applied after clipping may introduce some artifacts, namely
...
\textbf{The automatic level control (ALC)}

At the very end of the TX audio signal chain, it has to be \textit{guaranteed} that the baseband
signal does not exeed FS. The ALC does nothing if the signal at the end of the chain stays below FS,
...
predistortion (PURESIGNAL), the whole range of these values is used when auto-levelling the
RX feedback signal.
...
of piHPSDR, since in later runs the default value will be overwritten anyway when reading the settings (the props
file).
...
\end{figure}
 ...
 because then the necessary adjustement to the open-collector settings are automatically made.
 Note that when
...
frequent TX buffer underruns especially for a  sloppy network connection, a too high value does not
only delay the TX signal but also risks TX buffer overflows. Experience in the HL2  discussion group
seems to be that the defaul value of 20 msec of the HL2 firmware
...
left corner in yellow color. If \rett{Display Warnings} is checked in the same menu, then in addition to
the usual warnings, also an underrun or an overflow of the FPGA TX sample buffer is reported.
...
the RadioBerry, local loopback devices are queried for protocol-2.
...
that specic branch (either the master ``current'' branch or a release branch).

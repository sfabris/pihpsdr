...
at compile time) is stated, in this case, GPIO, MIDI, SATURN. These are compile-time options that are detailed in Appendix
\ref{sec:compiletime}.
...
closes and on sees that in the toolbar menu now reappearing (Fig. \ref{fig:ToolbarMenu4}), the third button in the second
...
\caption{The Encoder menu for the V2 controller}
...
\caption{The Encoder menu for the V1 controller}
...
\caption{The Switches menu for the V2 controller.}
...
There is a number of compile-time options that are shown on the initial screen. If you are using a binary version, 
you have what you have. If you compile from the sources, you can choose which options you want (or want not).
...
This ,,trick'' takes care the further compilations use the file \texttt{GNUmakefile} rather than \texttt{Makefile},
and changing the compile time options is accomplished by changing the file \texttt{GNUmakefile} with a text editor.
...
If you want to use the Controller2 or the G2 controller, the Raspberry Pi I2C interface
...
the source code: If you want to use the Controller2 or the G2 controller, the Raspberry Pi I2C interface
...
Note the API/ABI version (0.8). If you see 0.7 then you probably have installed SoapySDR on your computer from
the standard repositories, and this is not going to work. The \texttt{libinstall.sh} command  has been designed 
...
\textit{This procedure has been tested on a  fairly recent MacBookAir (2020 model, M1 Silicon CPU, running MacOS
...
in the Applications folder. It is suggested to drag this application into the ,,dock'' so you have quick access.
Although MacOS has a complete Unix ,,under the hood'', one needs additional software to to Linux-style
programming. The most popular such ,,Unix enabler'' packages are \texttt{MacPorts} and \texttt{HomeBrew}, I am using
...
procedure works in this case as well and does no harm. In addition to
 ...
 If you want to use SoapySDR, please check the Soapy installation. Open a new terminal window and simply type
 the command (be careful to use the correct capitalization!)

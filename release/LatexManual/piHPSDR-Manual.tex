...
is USB using the filter width is about 2.7 kHz. If the filter
width has not been temporarily modified (for fighting QRM)
using a controller or panel button, then this agrees with the
name of the chosen filter except for the two variable filters
(\texttt{Var1} and \texttt{Var2}) which are there for each mode
(see the \bltt{Filter} menu). So note the VFO bar reports the
actual filter width and not the name of the chosen filter.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{piHPSDR menus: introduction}

\section{piHPSDR menus}
...
that they cover a relevant part of the piHPSDR window. If you have a large display, you can move the menu
...
is included in the list. Be prepeared that the layout looks not OK if you choose a font
that is not installed on your system. The \rett{Roboto Mono} font can be downloaded
free of charge from the internet, installation on LINUX usually requires to create
a directory \texttt{\$HOME/.fonts} (if it not exists already), copy the downloaded
font files (\texttt{*.otf} or \texttt{*.ttf} to that directory and run the command
\texttt{fc-cache -v -f} in a terminal window. Installation on MacOS is usually done
simply by double-clicking a \texttt{*.ttf} file (this opens the FontBook application)
...
\subsection{Display Menu: Peaks}

\rett{Show Peak Numbers on Panadapter:} This enables peak detection.

\rett{Show Peaks in Passband Only:} If the box is checked only the peaks in the 
chosen filter passband 
are shown.

\rett{Hide Peaks Below Noise Floor:} This hides the peaks below the noise floor.

\rett{Peaks to Show:} This determines how many peaks will be shown. There can always be less,
 but no more than this number.

\rett{Panadapter Ignore Adjacent Peaks:} This number determines if nearby peaks are shown or are
 supressed. The higher the number the closer the peaks can be to eachother. It's a divisor of the 
 full window width so setting it to 1 will mean only one peak can occupy the entire window,

\rett{Panadapter Noise Floor Percentile:} This is the percentile setting that defines the noise floor. 
If the \rett{Hide Peaks Below Noise Floor} box is ticked peaks below this value will not be shown.

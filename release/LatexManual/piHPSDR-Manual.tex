...
If the speech processor is used simulataneously with the continuous frequency
compressor (CFC), there is a yellow \rett{CprCfc} at this place, and if
the CFC is used but not the speech processor, the symobol is \rett{CFC on}.
...
or the continous frequency compressor. For normal speaking into the microphone,
the average ALC value will be negative even if the Mic gain slider is adjuste
...
will probably want to have the same settings for USB and LSB. Therefore,
 ...
 DIGU/DIGL. If modified settings are stored for a mode that belongs to one of the three
 groups, the updated settings are copied to the settings for the other
 modes the same group. For example, when change noise reduction parameters
 or equalizer settings for the active receiver when running USB,
  these changes will also become effective when later changing the mode to LSB.
...
the \rett{CFC settings} for controlling the continous frequency compressor (CFC), and the
...
correction, the TX auto leveler and the TX phase rotator.
...
In the CFC sub-menu, the settings for the continous frequency compressor can be made.
As the name indicates, the compression can be chosen frequency dependent. As for the
equalizer, one can specify corner frequencies with a given compression, between corner
...
the speech processor, using the CFC automatically enables the TX auto-leveler and phase rotator.
...
\rett{Add Freq.-Indep. Gain} If using the post-compression equaliser, this spin button speficies an
\textit{additional} frequency-idependent gain applied to all frequencies.

In the lower part of the menu, you find ten groups of three spin buttons that specify a frequency/level/gain
triple. The compression level and gain given (if the post-compression equalizer is used) applies to that
...
\rett{Use Side Channel Filter} With this check box, ths side channel filter can be enabled of disabled.
...
\rett{Trigger Relese Time (ms)} This is the time used for closing the noise  gate. To avoid plops in the
audio, the gate is not opened instantly but with a raised cosine characteristic. The time for closing the
gate can usually be chosed much larger than for  opening.
...
DIGU or DIGL, the equaliser settings for DIGUDIGL become effective

...
,,Sequence error'' message appears in the panel of the active receiver for two seconds.
...
\rett{PTTOUT} (Output, active high). This output indicates with a high level that piHPSDR is in the
...
\rett{CWOUT} (Output, active high). This output monitors the state of the built-in iambic keyer
(high = key down, low = key up), and can be used to drive external hardware with a tone generator
that generates a low-latency side tone, mixes it with the RX audio output, and feeds the combined
signal to the head phone.

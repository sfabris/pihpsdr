...
to about +10 dB. The reason is simply, that
for most audio gear one takes care that one stays below -10 dBFS (FS stands for full scale). On the
other hand, the audio input to the TX chain should scratch at the FS limit such that TX compression
or adaptive predistortion (PURESIGNAL) works best. Part of this gap may be closed when using
a compressor since this automatically involves an auto-leveler.  The best
...
the peak ALC value is substantially below zero during normal speech.
